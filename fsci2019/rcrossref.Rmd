...
knitr::opts_chunk$set(echo = TRUE,tidy.opts=list(width.cutoff=5000),tidy=FALSE, cache = TRUE)
library(kableExtra)
library(knitr)
library(tidyverse)
library(tibble)
...
`rcrossref` is a package developed by [Scott Chamberlain](https://scottchamberlain.info/), Hao Zhu, Najko Jahn, Carl Boettiger, and Karthik Ram, part of the  [rOpenSci](https://ropensci.org/) set of packages. It serves as an interface to the Crossref API.
...
listviewer::jsonedit(plosone_details)
```
...
```{r crjournals1x, eval=TRUE,cache=TRUE}
...
To get metadata for the publications themselves rather than data about the journal, we will again use the `plosone_issn` value in the `issn =` argument to `cr_journals`, but we now set `works = TRUE`. The default number of articles returned is 20, but you can increase or decrease that with the `limit` argument. The max limit is 1000, but you can get more using the `cursor` argument (see below). As above, we use `pluck()` to return only the list element called "data", which is where the meat is.
...
For a full package on converting between citation formats, see the rOpenSci package [`handlr`](https://cran.r-project.org/web/packages/handlr/handlr.pdf).
