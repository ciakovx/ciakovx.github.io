...
Because the newest version of `rorcid` came out in June, new features and bug fixes have since been added. We may need to install the latest version directly from GitHub by using the following code. I'll let you know in class if this is necessary.
...
```{r simplexx, eval=TRUE, cache=TRUE}
carberry <- rorcid::orcid_search(family_name = 'carberry',
                                 rows = 50)
carberry
```
...
Note that most institutions in ORCID have Ringgold IDs, but you have to [register with Ringgold](https://www.ringgold.com/identify-online-guests/) to search in their registry. Sometimes different entities on campus will have separate Ringgold IDs; you may consider contacting Ringgold to get the full list of your institution's identifiers. [GRID is open for searching](https://www.grid.ac/institutes/grid.268117.b), but not all institutions have a GRID in ORCID. 
...
clarke <- rorcid::orcid(family_name = 'family-name:iakovakis AND email:*@okstate.edu')
clarke
```

# Getting all ORCIDs affiliated with an institution
...
There are 1,402 records. Next we will first create a numeric vector using `seq` that starts with 0 and ends with 1402, adding 200 to each value. 
...
If you look at the result for carberry_person in the Environment Pane in R Studio, you will see it returned a List of 1. 
...
```{r employment6, eval=TRUE, cache=TRUE}
